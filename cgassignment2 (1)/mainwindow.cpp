...
for (i=0;i<vertex-1;i++)
{
    ...
    if(dy==0.0) slope_array[i] = 1.0;

    if(dx==0.0) slope_array[i] = 0.0;

    if((dy != 0.0) && (dx != 0.0)){
        slope_array[i] = float(dx/dy);
...
for(int y=ymin; y<ymax ; y++){
    k = 0;
    for(i=0;i<vertex-1;i++){
        if(((array_2[i] <= y) && (array_2[i+1]>y)) || ((array_2[i] > y) && (array_2[i+1] <= y))){
            xi[k] = int(array_1[i] + (slope_array[i] * (y - array_2[i])));
            k++;
        }
    }
    for(j=0;j<k-1;j++)
...
// this function will be used to returning sign of the increment value
